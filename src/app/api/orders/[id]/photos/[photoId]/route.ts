...
export async function DELETE(
  ...
  try {
    ...
    // Tenter de supprimer le fichier physique en utilisant les clés stockées (non bloquant)
    try {
      const { deleteOrderPhoto } = await import('@/lib/storage/upload')
      if (photo.storageKey) {
        // Utiliser les clés stockées en DB pour une suppression fiable
        const thumbKey = photo.thumbnailKey ?? photo.storageKey.replace('.webp', '_thumb.webp')
        await deleteOrderPhoto(photo.storageKey, thumbKey)
      } else {
        // Fallback : extraire la key depuis l'URL (photos sans storageKey)
